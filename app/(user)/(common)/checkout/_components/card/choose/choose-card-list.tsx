...
function ChooseCardList() {
	...
	const { isPending, isLoading, isError, data, error } = useQuery({ queryKey: ["choose-card-list"], queryFn: () => getUserSavedCards() });
	...
	return (
		<>
			...
			<fieldset className="fieldset gap-4">
				{data.cards.map((card: { _id: string; cardNumber: string; cardType: string }) => (
					<label
						key={card._id}
						...
						/>
						<div>
							<p className="font-bold truncate max-w-[400px]">{card.cardNumber}</p>
							<p className="text-xs opacity-50">{card.cardType}</p>
						</div>
					</label>
				))}
			</fieldset>
